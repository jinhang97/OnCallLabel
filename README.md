...
![标签未聚焦时自动半透明，不影响浏览器使用](readme_images/other_app_off_focus.png)
...
- **悬浮球模式**
  - 双击标签 → 收起为一个悬浮球（跟随当前主题配色，中央显示 TODO 清单图标）
  - 双击悬浮球 → 展开恢复为完整标签
  - 拖动悬浮球到屏幕边缘附近释放 → 自动半圆贴边隐藏
  - 鼠标悬浮在贴边的半圆上 → 浮现整个圆，移开后自动缩回
- **编辑文本**
  - 按住 `Ctrl` 双击标签进入编辑
...
| 收起为悬浮球 | 在标签上**双击**鼠标左键                 |
| 展开为标签  | 在悬浮球上**双击**鼠标左键                |
| 移动悬浮球 | 在悬浮球上按住鼠标左键直接拖动                |
| 悬浮球贴边隐藏 | 拖动悬浮球到屏幕边缘附近释放，自动收为半圆          |
| 唤出贴边悬浮球 | 鼠标悬浮在半圆上浮现整个圆，移开后自动缩回          |
...
- **悬浮球模式**：开关。开启 = 收起为悬浮球；关闭 = 展开为完整标签，切换即时生效。
- **开机自动启动**
...
│   ├── renderer.js      # 标签交互：拖动、缩放、编辑、快捷键、悬浮球模式
